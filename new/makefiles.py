...
for file in directory:
	...
	for line in iter(layout):
		outfile.write(line.replace("%CONTENT%",content).replace("%KEYWORDS%",keywords).replace("%DESCRIPTION%",description).replace("%TITLE%",title).replace("%DESC%",desc).lstrip().rstrip())
	layout.seek(0)
	infile.close()
	outfile.close()
layout.close()
